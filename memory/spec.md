...
- `GET /invoices/next-number` (preview; authoritative number assigned at finalize), `GET /invoices?search=&date_from=&date_to=&page=1&limit=10` (database-paginated `{ invoices, page, limit, total, total_pages }`, newest first), `POST /invoices` (atomic all-or-nothing stock deduction guarded `stock >= qty`, rollback on failure → 409 "Insufficient stock available for X."; discount > subtotal → 400; bad date → 400), `GET /invoices/{id}`.
...
- `GET /dashboard` — `total_products`, `total_stock`, `low_stock`, `out_of_stock`, `today_sales`, `today_invoice_count`, `sale_count` (sum of invoice item quantities; server-side "today" via `lib.dates.today_iso()` UTC).
- `GET /reports/stock-sales?month=YYYY-MM` keeps the monthly report; `GET /reports/stock-sales?date=YYYY-MM-DD` returns the daily report with the same summary/product/chart shape plus `report_type` and `selected_date`. Daily opening stock reverses received/sold movements from the selected date through the current balance, and invoice snapshots provide historical selling prices when no matching sold transaction exists. Report sales amounts allocate saved invoice discounts once across their item lines.
- Business dates default to `Asia/Kolkata` via `lib.dates.today_iso()` unless `APP_TZ` is configured. New product creation records positive initial stock as one `received` stock transaction; Add Stock continues recording separate received transactions.
- `GET/PUT /settings`. Template probe endpoints `GET /`, `POST/GET /status` remain.

## Invoice creation flow (v2)
InvoiceBuilderModal (`components/invoices/InvoiceBuilderModal.tsx`): opened from Dashboard button / Products Sell / History buttons. Fresh form each open (`preselectProductId` preselects Sell product). Issued To (name required), auto number (preview; assigned at finalize), invoice date is always the server's today and is not user-editable, item rows via searchable ProductPicker (auto code/price), qty validated against stock minus same product in other rows → inline "Insufficient stock available. Only N left in stock." Finalize → ConfirmDialog → POST → snapshot InvoiceViewModal opens over the builder with Print.
